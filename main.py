...
def save_to_db(order: dict):
    ...
    for food_item, quantity in order.items():
        ...
        item_price = db_helper.get_price( food_item)
        total_price = int(item_price) * int(quantity)
        rcode = db_helper.insert_order_item(next_order_id,item_id,quantity, total_price)
        print("rcode: ", rcode)
        if rcode == -1:
            return -1
...
def complete_order(parameters: dict, session_id: str):
    if session_id not in inprogress_orders:
        ...
        if order_id == -1:
            ...
            print("order total price: ", order_total)
